...
void AtomSerialPwm_Init(IfxGtm_Atom_Timer* mytimer,float32 frequency, uint32 startingDataStream, uint32 bitShift, IfxGtm_Atom_ToutMap* atomPin)
{
   ...
   IfxGtm_Atom_TimerSerial_init(mytimer, &atomSerialConfig, bitShift);
...
{
  IfxGtm_Atom_Timer_disableUpdate(myatomtimer);
  IfxGtm_Atom_Ch_setCompareOneShadow(myatomtimer->atom, myatomtimer->timerChannel, DataStream);
  IfxGtm_Atom_Timer_applyUpdate(myatomtimer);
}
...
{
  IfxGtm_Atom_Timer_disableUpdate(myatomtimer);
  IfxGtm_Atom_Ch_setCompareZeroShadow(myatomtimer->atom, myatomtimer->timerChannel, Shifts);
  IfxGtm_Atom_Timer_applyUpdate(myatomtimer);
}
