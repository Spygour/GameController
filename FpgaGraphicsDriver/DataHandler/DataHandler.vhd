...
ENTITY DataHandler IS
    PORT (
        ...
        DataHandler_DebugLeds : OUT STD_LOGIC_VECTOR (7 DOWNTO 0);
...
ARCHITECTURE rtl OF DataHandler IS
    ...
    SIGNAL DataHandler_SdRamStoreState_reg : DATAHANDLER_STORE_STATE := DATA_1;
...
BEGIN
    ...
    BEGIN
        IF (DataHandler_Reset_Sync = '1') THEN
            ...
            DataHandler_SdRamStoreState_reg <= DATA_1;
            DataHandler_DebugLeds <= "00000000";
        ELSIF rising_edge(DataHandler_SdRamClk) AND DataHandler_PllLocked = '1' THEN
            CASE DataHandler_SdRamHandlerState IS
                ...
                WHEN RESTART_WRITE =>
                    IF DataHandler_SdRamStoreState = NULL_STATE THEN
                    ...
                    ELSIF DataHandler_SdRamStoreState = WAIT_INDEX_0 THEN
                        -- Restart the Write
                        DataHandler_DataColsOutput <= DataHandler_WriteDataBuffer(0);
                        DataHandler_WriteDataBufferIdx <= 1;
                        DataHandler_SdRamHandlerState <= WAIT_START;
                        DataHandler_SdRamStoreState_Reg <= DataHandler_SdRamStoreState;
                        DataHandler_WrEn <= '1';
                        DataHandler_SdRamHandlerState <= WAIT_START;
                    END IF;

                WHEN WAIT_START =>
                    IF DataHandler_WrFinish = '0' THEN
                ...
                WHEN WAIT_WRITE =>
                    IF DataHandler_WrFinish = '1' THEN
                        IF DataHandler_WriteDataBufferIdx = 64 THEN
                            DataHandler_SdRamHandlerState <= RESTART_WRITE;
                ...
                WHEN CHECK_DATA_AVAILABLE =>
                    IF (DataHandler_SpiWordsReg + 1) < DataHandler_Words THEN
                        DataHandler_ColsAddress(8 DOWNTO 1) <= unsigned(DataHandler_ReadDataWord(0)(15 DOWNTO 8));
                        DataHandler_DebugLeds <= DataHandler_ReadDataWord(0)(15 DOWNTO 8);
                        DataHandler_ColsAddress(0) <= '0';
                        ...
                        -- Increase the address for the x y axis
                ...
                WHEN WAIT_READ =>
                    IF DataHandler_RdFinish = '0' THEN
                        -- DEACTIVATE THE READ
                        DataHandler_RdEn <= '0';
                ...
                WHEN READ_DATA =>
                    IF DataHandler_RdFinish = '1' THEN
                ...
                WHEN STORE_DATA =>
                    ...
                    IF DataHandler_RdFinish = '1' THEN
                        ...
                        --DataHandler_DebugLeds <= DataHandler_DataColsInput(1)(15 DOWNTO 8);
        ...
        ELSIF rising_edge(DataHandler_SdRamClk) AND DataHandler_PllLocked = '1' THEN
            CASE DataHandler_SdRamStoreState IS
                WHEN DATA_1 =>
                    IF DataHandler_WriteDataBufferIdx = 64 THEN
                        -- store here
                        DataHandler_WriteDataBuffer <= (("0101010111011111", "0000000000000000"), ("0101010111011111", "0101010100000000"), ("0101010111011111", "1010101000000000"), ("0101010111011111", "1111111100000000"), ("0101010111111111", "0000000000000000"), ("0101010111111111", "0101010100000000"),
                ...
                WHEN DATA_2 =>
                    IF DataHandler_WriteDataBufferIdx = 64 THEN
                        -- store here
                        DataHandler_WriteDataBuffer <= (("1010101010011111", "0000000000000000"), ("1010101010011111", "0101010100000000"), ("1010101010011111", "1010101000000000"), ("1010101010011111", "1111111100000000"), ("1010101010111111", "0000000000000000"), ("1010101010111111", "0101010100000000"),
                ...
                WHEN DATA_3 =>
                    IF DataHandler_WriteDataBufferIdx = 64 THEN
                        -- store here
                        DataHandler_WriteDataBuffer <= (("1111111101011111", "1010101000000000"), ("1111111101011111", "1111111100000000"), ("1111111101111111", "0000000000000000"), ("1111111101111111", "0101010100000000"), ("1111111101111111", "1010101000000000"), ("1111111101111111", "1111111100000000"),
                ...
                WHEN END_STORE_DATA =>
                    IF DataHandler_WriteDataBufferIdx = 64 THEN
                        DataHandler_SdRamStoreState <= NULL_STATE;
                    END IF;

                WHEN WAIT_INDEX_0 =>
                    IF DataHandler_WriteDataBufferIdx = 3 THEN --data has been stored to the actual WriteDataBuffer
